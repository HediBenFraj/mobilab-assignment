...
describe('BankAccountController', () => {
  ...
  const mockBankAccountService = {
    ...
    update: jest
      ...
      .mockImplementation((id: string, dto: UpdateBankAccountDto) =>
        Promise.resolve({
          _id: id,
          ...BankAccountDocumentsArray.find((e) => e._id === id),
          ...dto,
        }),
      ),
    remove: jest
  ...
  it('GET /bank-account/:id with non Existing Id should return 400', () => {
    return supertest(app.getHttpServer()).get('/bank-account/sdf').expect(404);
  });

  it('GET /bank-account/:id with an Existing Id should return 200', () => {
    return supertest(app.getHttpServer())
      .get('/bank-account/6377a0c7ec5b41b08e8b527d')
      .expect(200)
      .expect(
        BankAccountDocumentsArray.find(
          (e) => e._id === '6377a0c7ec5b41b08e8b527d',
        ),
      );
  });
  ...
  it('GET /bank-account/owner/:id with an existing ownerId should return 200 with bank account', () => {
    return supertest(app.getHttpServer())
      .get('/bank-account/owner/some owner id')
      .expect(200)
      .expect(
        BankAccountDocumentsArray.filter((e) => e.ownerId === 'some owner id'),
      );
  });

  it('PATCH /bank-account/:id with a non existing id should return 404', () => {
    return supertest(app.getHttpServer())
      .patch('/bank-account/qsd')
      .expect(404);
  });

  it('PATCH /bank-account/:id with an existing id should return 200', () => {
    const foundBankAccount = BankAccountDocumentsArray.find(
      (e) => e._id === '6377a0c7ec5b41b08e8b527d',
    );
    return supertest(app.getHttpServer())
      .patch('/bank-account/6377a0c7ec5b41b08e8b527d')
      .send({ name: 'some new name' })
      .expect(200)
      .expect({ ...foundBankAccount, name: 'some new name' });
  });

  it('DELETE /bank-account/:id with a non existing id should return 404', () => {
    return supertest(app.getHttpServer())
      .delete('/bank-account/qsd')
      .expect(404);
  });

  it('DELETE /bank-account/:id with an existing id should return 404', () => {
    return supertest(app.getHttpServer())
      .delete('/bank-account/6377a0c7ec5b41b08e8b527d')
      .expect(200)
      .expect(
        BankAccountDocumentsArray.find(
          (e) => e._id == '6377a0c7ec5b41b08e8b527d',
