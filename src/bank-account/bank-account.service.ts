...
export class BankAccountService {
  ...
  async findOne(id: string): Promise<BankAccount> {
    ...
    try {
      ...
      if (error.message.includes('Cast to ObjectId failed'))
        throw new InvalidInputException('Invalid bank account id');
    }
  ...
  async update(
    ...
    return await this.BankAccountModel.findByIdAndUpdate(
      id,
  ...
  async patch(
    ...
    return await this.BankAccountModel.findByIdAndUpdate(id, patchAccount, {
      new: true,
    }).session(session);
  }
  ...
  async updateBankAccountBalances(
    ...
    const accountsHaveSameCurrency =
      senderAccount.currency === recieverAccount.currency;
    if (accountsHaveSameCurrency) {
      ...
      conversionObject = await this.conversionService.convert(
        amount,
        senderAccount.currency,
        recieverAccount.currency,
      );

      newSenderAccountBalance = senderAccount.balance - amount;
      newRecieverAccountBalance =
        recieverAccount.balance + conversionObject.convertedAmount;
    }

    this.patch(senderAccount.id, { balance: newSenderAccountBalance }, session);
    this.patch(
      recieverAccount.id,
      { balance: newRecieverAccountBalance },
      session,
    );
