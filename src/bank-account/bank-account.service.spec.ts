...
describe('BankAccountService', () => {
  ...
  const mockRequestServcie = {
    getUserId : jest.fn().mockImplementation(()=> {return "someId"})
  }

  const mockConversionService = {
    convert : jest.fn().mockImplementation((amount, fromCurrency, toCurrency)=>({
      amount,
      fromCurrency,
      toCurrency
    }))
  ...
  it('should return all bank accounts', async () => {
    jest.spyOn(BankAccountModel, 'find').mockReturnValueOnce(BankAccountDocumentsArray as any);
  ...
  it('should return one Account for an existing and valid objectId', async () => {
    ...
    jest.spyOn(BankAccountModel, 'findById').mockReturnValueOnce(BankAccountDocumentsArray.find(e => e._id === '6377ac4cfd59cc4418f4bcef') as any);
    const foundAccount = await service.findOne("6377ac4cfd59cc4418f4bcef")
    expect(foundAccount).toEqual(BankAccountArray.find(e => e._id === "6377ac4cfd59cc4418f4bcef"));
  });


  it('should throw an InvalidInputException if given a non valid ObjectId', async () => {
    await expect(service.findOne("4"))
    .rejects.toThrowError(InvalidInputException);
  });
  ...
  it('should covert money if currencies are different ',async ()=>{
    ...
    const {conversionObject} =await service.updateBankAccountBalances(converRequestBody.senderAccount,converRequestBody.recieverAccount,converRequestBody.amount)
  ...
  it('should update account balances',async ()=>{
    ...
    const updateBankAccountsOutput =await service.updateBankAccountBalances(converRequestBody.senderAccount,converRequestBody.recieverAccount,converRequestBody.amount)
 
 
    expect(updateBankAccountsOutput.newSenderAccountBalance).toEqual(converRequestBody.senderAccount.balance - converRequestBody.amount)
    expect(updateBankAccountsOutput.newRecieverAccountBalance).toEqual(converRequestBody.recieverAccount.balance + updateBankAccountsOutput.conversionObject.convertedAmount)
  })
